...
const int numImages = 300;
const int numPoints = 300;
double cornerThreshold = .001;
...
// const char imDir[] = "/home/likewise-open/ASTROBOTIC/dylan.koenig/Documents/datasets/gravity_offload/left_";
// const char imDirR[] = "/home/likewise-open/ASTROBOTIC/dylan.koenig/Documents/datasets/gravity_offload/right_";
...
const int windowHalfSize = 20*shrinkScale;
...
class comparePairPQ {
	public:
		bool operator() (const pair<double, Point2i>& pair1, const pair<double, Point2i>& pair2) {
...
int harris(const Mat&, vector<double>&, vector<Point2f>&, int&);
...
bool comparePair2(const pair<double, Point2i>&, const pair<double, Point2i>&);
bool outOfBounds(double, double, int, int);
// void invalidatePoint(int, Mat&, Mat&, Mat&);
void writeMat(const Mat&, const char *);
typedef priority_queue< pair<float, Point2i>, vector< pair<float, Point2i> >, comparePairPQ > pq_t;
...
void detectNewPoints(const Mat& im, imageData& imData, int& exitStatus) {
	...
	#ifdef ADAPTIVE_THRESH
	bool cornersSuccess = false;
	int harrisIterations = 0;
	while (!cornersSuccess) {
		if (harrisIterations > 5) {
			exitStatus = 1;
			return;
		}
		int cornersAboveThresh = 0;
		harris(im, imStrengths, imCorners, cornersAboveThresh);
		if (imCorners.size() < 1000) {
			cout << "found " << imCorners.size() << " corners with thresh = " << cornerThreshold << " threshold lowered, cats = " << cornersAboveThresh << endl;
			cornerThreshold -= cornerThreshold/4;
			imCorners.clear();
			imStrengths.clear();
		}
		else if (cornersAboveThresh > 60000) {
			cout << "found " << imCorners.size() << " corners with thresh = " << cornerThreshold << " threshold raised, cats = " << cornersAboveThresh << endl;
			cornerThreshold += cornerThreshold/4;
			imCorners.clear();
			imStrengths.clear();
		}
			...
			cornersSuccess = true;
			cout << "corners above thresh = " << cornersAboveThresh << endl;
		}
		harrisIterations++;
	}
	#endif

	#ifndef ADAPTIVE_THRESH
	int cornersAboveThresh = 0;
	if ((exitStatus = harris(im, imStrengths, imCorners, cornersAboveThresh))) {
		return;
	}
	#endif
...
int harris(const Mat& im, vector<double>& strengths, vector<Point2f>& corners, int& cornersAboveThresh) {
	Mat harrisImage;

	Mat imBlurred, Ix, Iy, Ix2, Iy2, IxIy;
	Mat im64;
	im.convertTo(im64, CV_64FC1);
	GaussianBlur(im64, imBlurred, Size(9,9), 1, 1, BORDER_DEFAULT);
	Sobel(imBlurred, Ix, -1, 1, 0, 1, 1, 0, BORDER_DEFAULT);
	Sobel(imBlurred, Iy, -1, 0, 1, 1, 1, 0, BORDER_DEFAULT);
	GaussianBlur(Ix.mul(Ix), Ix2, Size(11, 11), 1.5, 1.5, BORDER_DEFAULT);
	GaussianBlur(Iy.mul(Iy), Iy2, Size(11, 11), 1.5, 1.5, BORDER_DEFAULT);
	GaussianBlur(Ix.mul(Iy), IxIy, Size(11, 11), 1.5, 1.5, BORDER_DEFAULT);
	harrisImage = (Ix2.mul(Iy2)-IxIy.mul(IxIy)).mul(1/(Ix2+Iy2+double(pow(2.0,-52.0))));

	// cornerHarris(im, harrisImage, 3, 3, .04, BORDER_DEFAULT);
	...
	for (int rowSection = 0; rowSection < rowBins; rowSection++) {
		for (int colSection = 0; colSection < colBins; colSection++) {
			pq_t currBinPQ;
			const int rowStart = rowSection*rowDivide+halfSize;
			const int colStart = colSection*colDivide+halfSize;
			Mat currBinMat = harrisImage(Range(rowStart,rowStart+rowDivide),Range(colStart,colStart+colDivide));
			for (int row = 0; row < rowDivide; row++) {
				for (int col = 0; col < colDivide; col++) {
					currBinPQ.push(make_pair(currBinMat.at<double>(row,col), Point2i(colStart+col, rowStart+row)));
					if (currBinPQ.size() > (size_t) binLimit) {
						currBinPQ.pop();
					}
				}
			}
			for (int i = 0; i < binLimit; i++) {
				pair<double, Point2i> currPair = currBinPQ.top();
				corners.push_back(currPair.second);
				strengths.push_back(currPair.first);
				currBinPQ.pop();
			}
			//sort the vector and take 15 best corners
			// is there a way to grab just the floats and just the points out of this vector in order to assign them to the outputs without for loops?
			// sort(currBin.begin(), currBin.end(), comparePair2);
			// for (vector< pair<float, Point2f> >::iterator iter = currBin.begin(); iter != (currBin.begin()+binLimit); iter++) {
			// 	corners.push_back((*iter).second);
			// 	strengths.push_back((double) ((*iter).first));
			// }
		}
	}*/

	
	// vector sorting version (fastest currently)
	for (int rowSection = 0; rowSection < rowBins; rowSection++) {
		for (int colSection = 0; colSection < colBins; colSection++) {
			vector< pair<double, Point2i> > currBinPts (binLimit);
			const int rowStart = rowSection*rowDivide+halfSize;
			const int colStart = colSection*colDivide+halfSize;
			Mat currBinMat = harrisImage(Range(rowStart,rowStart+rowDivide),Range(colStart,colStart+colDivide));
			for (int row = 0; row < rowDivide; row++) {
				for (int col = 0; col < colDivide; col++) {
					double currCornerStrength = currBinMat.at<double>(row,col);
					if (currCornerStrength > (currBinPts.back()).first) {
						if (currBinPts.size() == (size_t) binLimit) {
							currBinPts[binLimit-1] = make_pair(currCornerStrength, Point2i(colStart+col, rowStart+row));
						}
						else {
							currBinPts.push_back(make_pair(currCornerStrength, Point2i(colStart+col, rowStart+row)));
						}
						sort(currBinPts.begin(), currBinPts.end(), comparePair2);
					}
				}
			}
			transform(currBinPts.begin(), currBinPts.end(), back_inserter(strengths),[&corners](const pair<double, Point2i> &currPair) -> double {
					corners.push_back(currPair.second);
	...
	/*
	// vector sorting all points version
	for (int rowSection = 0; rowSection < rowBins; rowSection++) {
		for (int colSection = 0; colSection < colBins; colSection++) {
			vector< pair<double, Point2i> > currBinPts (binLimit);
			const int rowStart = rowSection*rowDivide+halfSize;
			...
			sort(currBinPts.begin(), currBinPts.end(), comparePair2);
			transform(currBinPts.begin(), currBinPts.begin()+binLimit+1, back_inserter(strengths),[&corners](const pair<double, Point2i> &currPair) -> double {
					corners.push_back(currPair.second);
					return currPair.first;
				});
		}
...
bool comparePair2(const pair<double, Point2i>& pair1, const pair<double, Point2i>& pair2) {
	return pair1.first > pair2.first;
}
